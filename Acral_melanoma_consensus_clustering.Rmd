---
title: "acralMX_consensus_clustering"
...
# Select columns and make factor
metadata <- as.data.frame(metadata)
anno <- metadata %>% select(ID, TMB, Whole_Genome_Duplication) %>% 
                     mutate(Whole_Genome_Duplication = as.factor(Whole_Genome_Duplication)) %>% 
                     glimpse()
...
####--- Check dimensions for cola
dim(mt)
dim(muestras_anno)

# Run all partition methods 
consensus_results = run_all_consensus_partition_methods(mt, anno = muestras_anno, anno_col = colors)

# Or load the precomputed results object
#consensus_results <- readRDS("consensus_results.rds")

# Write report and analyze data - metrics and visual inspection of clusters for each k
cola_report(consensus_results, output_dir= "/home/user/Documents/ConsensusClustering")
...
# Test factors - TMB and WGD
test_to_known_factors(consensus_results)

# Plot membership heatmap
membership_heatmap(SD_PAM_K3, k = 3)

# Extract results for the best performing combination of methods (top-value and partitioning)
SD_PAM_K3 = consensus_results["SD", "pam"]

# Extracting classification
classification <- cbind(get_classes(consensus_results["SD:pam"], k = 3), get_membership(consensus_results["SD:pam"], k = 3))
...
# Signatures that are significantly different between subgroups
...
head(subset_signature)
dim(subset_signature)

# Saving object
saveRDS(subset_signature, "DE_gene_signature.rds")

####--- Creating heatmap
# Z-scores 
data_subset_signatures_z <- t(scale(t(subset_signature)))

# Alternatively, load the already computed z-score matrix
...
# Reorder and make factors 
metadata_ht <- metadata %>% select(BRAF, NRAS, KIT, NF1, Whole_Genome_Duplication, TMB, Ulceration, Breslow, Age, Sex, Stage) %>%
...
# Transposed matrix 
signature_ht_t <- t(data_subset_signatures_z)

# Or load the already computed object 
#signature_ht_t <- readRDS("transposed_signature_ht.rds")
#dim(signature_ht_t)
