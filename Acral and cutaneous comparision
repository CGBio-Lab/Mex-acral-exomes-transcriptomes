#----Acral and cutaneous comparision----- 
#Data: Newell et al, 2020

#----Data (counts)----
setwd("/home/kcontrerasramirez/Desktop/Newell_paty_paper ")
dir()
...
#----Extraction of WT for NRAS and BRAF-----
#WT
wt_braf <- ratio[(names(ratio) %in% wt$Donor)]
length(wt_braf)
wt_braf
...
#NRAS ---- They are all RAS genes.
head(nras)
dim(nras)
nras_values <- ratio[names(ratio) %in% nras$Donor]
length(nras_values)
nras_values

#wilcoxon test
wilcox.test(nras_values, wt_braf, alternative = "less")
